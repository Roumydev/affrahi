...
import { Clock, CheckCircle, Heart, Calendar, ArrowRight, Star, MapPin, Users, Sparkles } from "lucide-react";
      ...
      <div className="bg-white rounded-[32px] p-10 border border-gray-100 shadow-sm mb-10">
        ...
        {upcoming.length === 0 ? (
          <div className="text-center py-10">
            ...
            <Link
              href="/browse-halls"
              className="px-6 py-2.5 bg-[#8B1538] text-white rounded-lg text-sm font-bold font-montserrat hover:bg-[#6d102c] transition"
            >
              Browse Halls
            </Link>
          </div>
      ...
      <div className="bg-white rounded-[32px] p-10 border border-gray-100 shadow-sm">
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-3">
            ...
            <h3 className="text-2xl font-bold text-gray-800">Discover Halls</h3>
          </div>
          <Link
            href="/browse-halls"
            className="flex items-center gap-2 text-[#8B1538] font-semibold text-sm hover:gap-3 transition-all"
          >
            Browse All Halls <ArrowRight size={16} />
          </Link>
        </div>
        <p className="text-gray-400 text-sm mb-8 ml-14">
          Explore top-rated halls available for booking
        </p>

        {venues.length === 0 ? (
          <div className="text-center py-10">
            <p className="text-gray-400 mb-4">No halls available right now</p>
            <Link
              href="/browse-halls"
          ...
          <>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
              {venues.map((venue) => (
                <VenuePreviewCard key={venue.id} venue={venue} />
              ))}
            </div>
            <div className="flex justify-center">
              ...
              >
                See More & Book a Hall <ArrowRight size={18} />
              </Link>
            </div>
          </>
        )}
...
function VenuePreviewCard({ venue }: { venue: Venue }) {
  return (
...
function ReservationRow({
  ...
  return (
    <div className="flex items-center justify-between p-6 bg-[#F9F8F6] rounded-3xl">
      ...
      <span className={`px-5 py-1.5 rounded-full text-xs font-bold uppercase ${statusBg}`}>
        {status}
      </span>
    </div>
  );
