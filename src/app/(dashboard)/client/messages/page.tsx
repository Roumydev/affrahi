    ...
    axios.get("/api/messages").then((res) => {
      setMessages(
        res.data.messages.filter(
          (msg: Message) =>
            (msg.sender.id === myId && msg.receiver.id === selected.id) ||
            (msg.sender.id === selected.id && msg.receiver.id === myId),
        ),
      );
    });
  }, [selected, myId]);
    ...
    <div className="w-full h-[calc(100vh-120px)] flex gap-0 bg-white rounded-[32px] border border-gray-100 shadow-sm overflow-hidden">
      <div className="w-72 border-r border-gray-100 flex flex-col">
        <div className="p-6 border-b border-gray-100">
          ...
          <p className="text-xs text-gray-400 mt-1">{m.yourOwners}</p>
        </div>
          ...
          {contacts.length === 0 ? (
            ...
            contacts.map((c) => (
              <button
                ...
                className={`w-full flex items-center gap-3 px-5 py-4 hover:bg-gray-50 transition text-left ${selected?.id === c.id ? "bg-[#F9F1F3] border-r-2 border-[#8B1538]" : ""}`}
              >
      ...
      <div className="flex-1 flex flex-col">
        {!selected ? (
          <div className="flex-1 flex items-center justify-center text-gray-400 flex-col gap-3">
            ...
            <p className="font-medium">{m.selectContact}</p>
          </div>
        ) : (
          <>
              ...
              {messages.length === 0 ? (
                ...
                messages.map((msg) => {
                  ...
                  return (
                    ...
                    >
                      <div
                        className={`max-w-xs lg:max-w-md px-4 py-3 rounded-2xl text-sm ${isMe ? "bg-[#8B1538] text-white rounded-br-sm" : "bg-gray-100 text-gray-800 rounded-bl-sm"}`}
                      >
                        <p>{msg.content}</p>
                        <p
